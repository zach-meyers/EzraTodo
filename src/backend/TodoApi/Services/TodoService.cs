...
public class TodoService : ITodoService
{
    ...
    public async Task<IList<TodoResponse>> GetTodos(GetTodosRequest request)
    {
        ...
        if (request.CreatedDateFrom.HasValue)
            query = query.Where(t => t.CreatedDate >= request.CreatedDateFrom.Value);
        if (request.CreatedDateTo.HasValue)
            query = query.Where(t => t.CreatedDate <= request.CreatedDateTo.Value);
        if (!request.Tag.IsNullOrWhiteSpace())
            query = query.Where(t => t.TodoItemTags.Any(tt => tt.Tag == request.Tag));
    ...
    public async Task<TodoResponse> UpdateTodo(int id, UpdateTodoRequest request)
    {
        var userId = _httpContextAccessor.GetUserId();
